...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2022/09/01 15:53:37 by fcil              #+#    #+#             */
/*   Updated: 2022/09/01 16:48:37 by fcil             ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
{
	t_redir_undo	*redir_undo;

	redir_undo = malloc(sizeof(t_redir_undo));
	if (redir_undo == NULL)
		return (NULL);
...
static t_redir_undo	*redir_undo_content(t_list *redir_undo)
{
	return ((t_redir_undo *)(redir_undo->content));
}
	...
	while (iter)
	{
		...
		else if (fd_repl_dup != -1)
		{
			if (dup2(fd_repl_dup, fd_repl) == -1)
				status = print_error_errno(NAME, NULL, NULL);
			close(fd_repl_dup);
		}
		iter = iter->next;
	}
